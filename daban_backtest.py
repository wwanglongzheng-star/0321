"""
打板策略 · 历史回测框架（v8.4 赚钱效应自适应 + 第2天强制止损版）
================================================
策略说明：
...
DAY2_FORCE_STOP_LOSS   = -0.025  # 第2天收盘亏损超此值则第3天开盘强制止损出场（-2.5%）
...
SIGNAL_SCORE_MIN_FIRST    = 72.0   # ★v8.3: 从70提升至72（23%胜率验证高门槛有效，进一步过滤中等首板）
...
WEAK_MONTHS               = [3, 6, 7, 8, 10, 11]  # ★v8.3扩展：6月也加入（胜率25.5%偏弱）
...
STRONG_MONTHS              = [9]         # 强势月份：9月（历史62.2%胜率）
STRONG_MONTH_POSITION_RATIO = 1.5        # 强势月份仓位系数（1.5=加仓50%，最多到15%）
...
def simulate_exit(df: pd.DataFrame, entry_idx: int, entry_price: float,
    ...
    出场逻辑（v8.4 动态自适应版）：
      ...
      3. ★ v8.4 第2天强制止损：收盘亏损>2.5% 且非缩量洗盘 → 次日开盘强制出
      4. ★ v4.0 趋势止损：MA10，持仓≥5天生效
      5. ★ 优化⑤ T2后追踪止盈
      6. 智能跟踪止盈（T1后启动）
      7. ★ v8.4 强势延长持仓：连续2日强势则每次延长1天，最多+3天
      8. 超时平仓
      ★ v5.0：止损位随机化 + 止盈点反整数化
    ...
    for j in range(entry_idx, min(entry_idx + _hold_days + extra_hold_days, n)):
        row   = df.iloc[j]
        hi    = float(row["最高"])
        lo    = float(row["最低"])
        cls   = float(row["收盘"])
        ...
        if EXTEND_HOLD_ON_STRENGTH and not t1_hit:
            close_pos = float(row.get("close_pos", 0.5) or 0.5)
            if close_pos >= EXTEND_HOLD_CLOSE_POS_THRESH:
        ...
        # ── 6. 超时平仓 ───────────────────────────────────────────
        if days >= _hold_days + extra_hold_days:
            exit_p = round(cls * (1 - SLIP - COMMISSION), 4)
            return date, exit_p, "timeout", days

    # 数据不足，用最后一天收盘平仓
    last = df.iloc[min(entry_idx + _hold_days + extra_hold_days - 1, n - 1)]
    return str(last["date"])[:10], round(float(last["收盘"]) * (1 - SLIP - COMMISSION), 4), "timeout", _hold_days + extra_hold_days
...
def backtest_single(code: str, df: pd.DataFrame,
    ...
    if "洗盘抄底" in strategies:
        trades += backtest_washout_dip(code, df)
    if "日内抄底" in strategies:
