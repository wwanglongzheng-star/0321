"""
打板策略 · 历史回测框架（v9.0 竞价聚焦 + 出场革命 + 龙头过滤版）
================================================
策略说明：
...
AUCTION_GAP_ABORT       = -0.020   # ★v9.0: 入场当天低开>2%止损（比通用-3%更严）
...
def simulate_exit(df: pd.DataFrame, entry_idx: int, entry_price: float,
    ...
    for j in range(entry_idx, min(entry_idx + AUCTION_ZT_TRAIL_MAX_DAYS if (is_auction and AUCTION_ZT_TRAIL_ENABLE) else entry_idx + _hold_days + extra_hold_days + 1, n)):
        row   = df.iloc[j]
        hi    = float(row["最高"])
        lo    = float(row["最低"])
        cls   = float(row["收盘"])
...
def backtest_auction_board(code: str, df: pd.DataFrame) -> list:
    ...
    for i in range(1, n - 1):
        ...
        atr_stop = float(row.get("atr_stop_pct", STOP_LOSS) or STOP_LOSS)
        _trade_rng = random.Random()
        # ★ v9.0: 传入 is_auction=True，启用竞价专用止损+涨停跟踪出场
        exit_date, exit_p, reason, hdays = simulate_exit(df, i + 1, entry_p, atr_stop, _trade_rng, _dyn_hold, is_auction=True)
        pnl = (exit_p - entry_p) / entry_p
