"""
打板策略 · 历史回测框架（v9.5 竞价极致 + 龙头锁定 + 连板梯队版）
================================================
策略说明：
...
v9.5 bug修复（基于v9.4全量498笔回测 · 修复SKIP_MONTHS未真正生效）：
  ★ 修复SKIP_MONTHS实际未过滤6/11月的bug：
    - v9.4的SKIP_MONTHS=continue 写在 simulate_capital() 的资金模拟循环里
    - simulate_capital() 只影响资金计算，trades列表早在信号回测主循环里生成
    - 导致月度统计中6月仍有13笔、11月仍有16笔（SKIP_MONTHS完全没生效）
    - 修复：在 backtest_auction() 和 backtest_dt_recover() 的入场处各加一行过滤
    - 预期效果：6/11月笔数清零，总样本减少约29笔，整体胜率应进一步提升

v9.4 数据驱动精准优化（基于v9.3全量536笔回测 · 三项聚焦优化）：
  ★ 停用竞价-连板-极限：全量回测胜率仅45.2%（>50%概率亏损），实盘心理难以承受
    - 虽然均收+2.42%（靠均盈+10.01%拉回），但高方差低胜率不适合实盘执行
    - 从AUCTION_WHITELIST中移除，后续积累更多样本再评估是否重启
  ★ 6月/11月直接跳过（新增SKIP_MONTHS机制）：
    - 6月: 43.8%胜率/均收+0.00%——减仓至×0.5也无收益，不如直接不做
    - 11月: 50.0%胜率/均收+0.21%——均收趋近于零，减仓毫无意义
    - 新增SKIP_MONTHS=[6,11]参数，入场前直接跳过这两个月份
  ★ 首板强势评分门槛提高至65分（新增SIGNAL_SCORE_MIN_AUCTION_STRONG=65）：
    - 全量回测首板强势64.8%/+2.45% vs 温和77.4%/+4.19%，差距悬殊
    - 原因：高开5~7%已消耗上涨空间，低评分强势信号几乎没有盈利空间
    - 专用更高门槛过滤低分强势信号，保留高评分强势信号（有实质利好支撑的）

v9.3 数据驱动精准优化（基于v9.2回测结果 · 修正三处反向调整 · 根治gap_abort）：
  ★ 回滚①：极限档 PREV_CLOSE_POS 0.93→0.90, PREV_VOL_RATIO 1.3→1.2
    - v9.2 提高门槛后极限档胜率从47.1%进一步降至43.8%，反向了，说明提高条件过滤掉了好样本
  ★ 回滚②：连板强势档量比下限 2.0→VOL_RATIO_LOW(1.5)
    - v9.2提高至2.0后，竞价-连板-强势已在白名单外（停用），此参数无实际影响，回滚保持整洁
    - 首板强势档量比 2.5→2.0（v9.2提高到2.5后，强势档胜率从61.8%跌至52.6%，进一步反向）
  ★ 根治gap_abort：新增入场当天close_pos过滤（AUCTION_ENTRY_CLOSE_POS_MIN=0.50）
    - 连续两版（v9.1/v9.2）gap_abort均贡献>50%总亏损（16~17笔全亏，均亏-5%）
    - 根本原因：入场当天收盘弱（close_pos<0.5）=主力当天出货，次日必然低开
    - 新增过滤：竞价入场当天close_pos<0.5直接跳过，从源头截断gap_abort来源
  ★ 1月极端弱势加强：新增EXTREME_WEAK_MONTHS=[1]，仓位系数0.5→0.3
    - v9.2加入1月后，本次回测1月胜率降至25%/-2.16%，比其他弱势月更差
    - 0.5仓位仍然不够，改为0.3（全场最低），实质上半退出1月行情
...
AUCTION_WHITELIST = [
    "竞价-首板-温和",    # v9.3全量: 77.4% / +4.19% ← 主力，核心策略
    "竞价-首板-强势",    # v9.3全量: 64.8% / +2.45% ← 次优，保留
...
AUCTION_EXTREME_PREV_CLOSE_POS  = 0.90  # ★v9.3: 回滚至0.90（v9.2提高到0.93反向，极限档胜率进一步下降至43.8%）
AUCTION_EXTREME_PREV_VOL_RATIO  = 1.2   # ★v9.3: 回滚至1.2（v9.2提高到1.3反向，过滤掉了好样本）
AUCTION_EXTREME_MAX_STREAK      = 3     # 最多允许连板天数（4+连板极限高开=强烈警惕）

# ── ★ v9.3 入场当天收盘位置过滤（gap_abort根治）────────────────────────
# 思路：竞价高开当天收盘弱（close_pos低）=主力当天出货，次日必然低开触发gap_abort
# 回测铁证：gap_abort连续两版（v9.1/v9.2）均贡献>50%总亏损，根本在于入场前选错了股
AUCTION_ENTRY_CLOSE_POS_MIN     = 0.50  # 入场当天收盘位置<0.5直接跳过（收在下半部=出货）
...
SIGNAL_SCORE_MIN_AUCTION  = 50.0   # 竞价策略最低评分（温和档）
SIGNAL_SCORE_MIN_AUCTION_STRONG = 65.0  # ★v9.4: 强势档专用门槛（首板强势64.8%胜率，提高门槛保质量）
...
WEAK_MONTHS               = [1, 2, 3, 7, 12]  # ★v9.4: 移出6月/11月（均收趋零，减仓已无意义，改为直接跳过）
# ★v9.3：1月极端弱势（57%胜率/+1.89%，v9.3降仓×0.3有效，保持）
WEAK_MONTH_POSITION_RATIO = 0.5          # 弱势月份仓位系数（0.5=减半）
EXTREME_WEAK_MONTHS       = [1]          # ★v9.3: 极端弱势月（1月）仓位更低
EXTREME_WEAK_MONTH_POS_RATIO = 0.3      # 极端弱势月仓位系数（0.3=仅3成）
# ★v9.4: 直接跳过月份（减仓也救不了的月份：均收≈0，6月43.8%/+0.00%，11月50%/+0.21%）
SKIP_MONTHS               = [6, 11]     # 直接跳过，不入场（减仓到0等效）
...
def backtest_auction_board(code: str, df: pd.DataFrame) -> list:
    ...
    for i in range(1, n - 1):
        ...
        if ENABLE_FACTOR_FILTER:
            ...
            # ★ v9.3：入场当天收盘位置过滤（gap_abort根治）
            # 逻辑：竞价高开当天若收盘位置低（close_pos<0.5），说明当天资金出货，次日大概率低开
            # 回测：gap_abort贡献50.8%总亏损(16笔全亏均亏-5.02%)，根本原因是当天就被砸
            if close_pos < AUCTION_ENTRY_CLOSE_POS_MIN:
                continue  # 当天收盘太弱=主力当天出货，次日低开止损，直接不入场
            ...
            if prev_is_zt:
                ...
                if tier == "温和":
                    ...
                    # ★v9.3: 竞价-连板-强势已停用（在白名单外），此处不会被执行
                    # 若未来重启，量比回归 VOL_RATIO_LOW(1.5)（v9.2提升至2.0反向，强势档胜率从61.8%跌至52.6%）
                    if vol_ratio < VOL_RATIO_LOW:  # ★v9.3: 回滚至1.5（v9.2改2.0反向了）
                        continue
                    if bomb_30d > FILTER_BOMB_HISTORY_MAX:
                        continue
                ...
                if tier == "温和":
                    ...
                    if vol_ratio < 2.0:  # ★v9.3: 维持2.0（首板强势本身就高风险，无涨停背景更严格）
                        continue
            ...
            if SIGNAL_SCORE_ENABLE and score_a < SIGNAL_SCORE_MIN_AUCTION:
                continue
            # ★v9.4: 强势档专用更高评分门槛（首板强势64.8%胜率，低分强势信号性价比差）
            if SIGNAL_SCORE_ENABLE and sub_strategy == "竞价-首板-强势" and score_a < SIGNAL_SCORE_MIN_AUCTION_STRONG:
                continue
        ...
        # ★v9.4: 直接跳过月份过滤（在真正的信号入场处过滤，才能影响月度统计）
        if WEAK_MONTH_POSITION_ENABLE and int(entry_date[5:7]) in SKIP_MONTHS:
            continue
        atr_stop = float(row.get("atr_stop_pct", STOP_LOSS) or STOP_LOSS)
        _trade_rng = random.Random()
...
def _calc_portfolio_equity(df: pd.DataFrame) -> pd.Series:
    ...
    for _, row in df.iterrows():
        ...
        if WEAK_MONTH_POSITION_ENABLE and _entry_month in EXTREME_WEAK_MONTHS:
            _pos_ratio = POSITION_SIZE * EXTREME_WEAK_MONTH_POS_RATIO  # ★v9.3: 极端弱势月(1月)仅3成仓
        elif WEAK_MONTH_POSITION_ENABLE and _entry_month in SKIP_MONTHS:
            continue  # ★v9.4: 直接跳过（6月43.8%均收+0.00%，11月50%均收+0.21%，减仓已无意义）
        elif WEAK_MONTH_POSITION_ENABLE and _entry_month in WEAK_MONTHS:
