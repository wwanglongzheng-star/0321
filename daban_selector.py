...
★ v8.4 灵活仓位与出场优化说明（同步自 daban_backtest.py v8.4）：
  ...
  ② 第2天强制止损机制：
     入场后第2天收盘浮亏>-2.5% 且非缩量洗盘 → 第3天开盘必须止损出场
     推送消息中明确标注止损纪律，防止持续坐穿板凳
  ③ 弱/强月份仓位联动：
     弱势月份（3/6/7/8/10/11月）仓位系数×0.5（Kelly结果自动减半）
     强势月份（9月）仓位系数×1.5（历史62.2%胜率，可以适当放大）
  ④ 强势延长持仓提示：
...
# 各策略历史回测参数（均为半Kelly保守估计）：
#   首板：胜率55%，盈亏比1.85x → Kelly≈30.7% → 高分半Kelly≈15%
#   连板：胜率35%（高风险），盈亏比2.2x  → Kelly≈9.5%  → 高分≈10%
#   竞价：胜率45%，盈亏比1.6x            → Kelly≈16.9% → 高分≈17%
#   反包：胜率40%，盈亏比1.8x            → Kelly≈7.2%  → 高分≈7%
#   洗盘/低吸：胜率50%，盈亏比1.5x       → Kelly≈16.7% → 高分≈17%
KELLY_PARAMS = {
    "首板":  {"win": 0.55, "ratio": 1.85},
    "连板":  {"win": 0.35, "ratio": 2.20},   # 高风险，谨慎
    "竞价":  {"win": 0.45, "ratio": 1.60},
    "反包":  {"win": 0.40, "ratio": 1.80},
...
# ★ v8.4：弱/强月份仓位系数（同步自 daban_backtest.py v8.3/v8.4）
# 回测验证：3/6/7/8/10/11月胜率偏低，9月胜率62.2%显著高于其他月份
WEAK_MONTH_POSITION_ENABLE  = True          # 启用弱势月份仓位减半
WEAK_MONTHS                 = [3, 6, 7, 8, 10, 11]   # 弱势月份
WEAK_MONTH_POSITION_RATIO   = 0.5           # 弱势月份Kelly结果×0.5
STRONG_MONTH_POSITION_ENABLE = True         # 启用强势月份加仓
STRONG_MONTHS               = [9]           # 强势月份：9月
STRONG_MONTH_POSITION_RATIO = 1.5           # 强势月份Kelly结果×1.5
...
WASHOUT_MIN_SCORE    = 55       # 洗盘信号最低评分门槛（低于此值不推送）
...
def kelly_position_advice(score: float, strategy: str = "", market_state: str = "",
    ...
    # ★ v8.4：月份仓位系数（弱势月份减半，强势月份加仓）
    month_note = ""
    _cur_month = beijing_now().month
    if WEAK_MONTH_POSITION_ENABLE and _cur_month in WEAK_MONTHS:
...
def push_signals(signals: list, phase: str, emotion: dict = None) -> None:
    ...
    footer = ("\n> ⚠️ 仅供参考，严格止损，注意仓位\n"
              ...
              "> 📌 第2天止损纪律：入场后第2天收盘浮亏>-2.5%且非缩量→第3天开盘必须止损出场\n"
...
def main():
    ...
    global _HEARTBEAT_PUSHED_HOURS
