...
# ★ v8.5：弱/强月份仓位系数（基于v8.4真实回测数据重校准）
# 回测新数据：8月胜率30.4%/10月胜率30.7%（各月最高），不该算弱势月；12月20.8%/-2.13%应加入
# 9月实测29.0%/-0.29%，仍略强于均值，系数从1.5降至1.2
WEAK_MONTH_POSITION_ENABLE  = True          # 启用弱势月份仓位减半
WEAK_MONTHS                 = [3, 6, 7, 11, 12]   # ★v8.5: 移出8/10月，新增12月
...
AUCTION_WHITELIST = [
    ...
    "竞价-连板-极限",    # 60.0% / +5.27% ← 超强（样本15笔，方向明确）
    # "竞价-连板-温和",  # 34.6% / -0.73% ← 停用
    # "竞价-连板-强势",  # 38.7% / -1.23% ← 停用
]
