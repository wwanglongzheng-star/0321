...
# ── ★ v10.1 保本风控配置（核心：保住本金 > 大概率赚钱）────────────────────────
# 设计原则：
#   1. 单笔最大仓位硬上限 → 任何信号都不允许超过；
#   2. 超早期预警（<4%）专用更低仓位 → 早期确定性不足，小仓试探；
#   3. 大盘极弱时全局仓位打折 → 熊市少做；
#   4. 炸板率高时仓位降档 → 市场分歧大，主力不护盘；
#   5. 单日信号数量上限 → 避免单日满仓集中踩雷；
#   6. 止损纪律：当天开仓当天浮亏>STOP_LOSS_INTRADAY立刻出。

# 单笔最大仓位（任何信号不可超过，防止重仓一只导致本金大幅回撤）
MAX_POSITION_PER_TRADE   = 15    # 单笔最大仓位 15%（≤10万账户实际上限）
MAX_POSITION_SUPER_EARLY = 8     # 超早期预警（涨幅<4%）最大仓位 8%（确定性低）
MAX_POSITION_NEAR_ZT     = 5     # 即将封板（涨幅>7%）最大仓位 5%（空间有限）

# 大盘极弱时仓位折扣（保本优先）
WEAK_MARKET_ZT_THRESHOLD = 30    # 涨停家数<30家时视为弱市（比停推阈值宽松，只降仓不停推）
WEAK_MARKET_POS_RATIO    = 0.6   # 弱市仓位折扣：Kelly建议×0.6
EXTREME_WEAK_ZT_THRESHOLD = 15   # 极弱（涨停<15家）：折扣更大
EXTREME_WEAK_POS_RATIO   = 0.35  # 极弱市仓位折扣：Kelly建议×0.35

# 炸板率高时仓位降档（分歧大=主力不护，拿住亏损概率高）
BOMB_RATE_REDUCE_THRESHOLD = 0.55  # 炸板率≥55%：仓位打8折
BOMB_RATE_REDUCE_RATIO     = 0.8
BOMB_RATE_STOP_THRESHOLD   = 0.70  # 炸板率≥70%：仓位打5折（极端弱市）
BOMB_RATE_STOP_RATIO       = 0.5

# 当日最大推送信号数上限（避免单日信号太多导致分散注意力+总仓过重）
MAX_DAILY_SIGNAL_PUSH    = 5     # 每天最多推送5个新信号（竞价/盘中合计）

# 盘中止损纪律（买入当天）
INTRADAY_STOP_LOSS_PCT   = -0.03  # 当天浮亏>3%立刻出，不持仓过夜
INTRADAY_STOP_LOSS_NEAR_ZT = -0.02  # 即将封板信号（利润空间小）止损更严：-2%
...
# ── ★ 直线拉升预警配置（v10.0 重构：更早预警，涨停前留足利润空间）──────
# 核心逻辑：捕捉量价齐升的"直线拉升"态势，在冲向涨停的途中尽早推送。
# 设计原则：宁早勿晚——涨2%时推，比涨8%时推多出约8%的利润空间。
# v10.0优化：
...
def get_realtime_quotes(codes: list = None) -> pd.DataFrame:
    """
    获取实时行情（v3.14 真正并发竞速版）。

    ★ 并发设计：2个接口同时启动（新浪+腾讯），任何一个成功立刻返回，不等其余接口。
    ...
    ★ 优先级机制：queue 里放 (priority, source, df)，主线程收到第一个成功结果
      后再等 0.5s 捞一次队列，新浪（priority=0）优先。
    ...
    #   通达信TCP(mootdx)接口在GitHub Actions环境常失败，已剔除。
    fetchers = [
...
def get_intraday_kline(code: str) -> pd.DataFrame:
    """
    获取当日分时数据（用于封板时间判断）。
    ★ v10.1：加并发超时保护（8s），境外环境 akshare 分时接口经常超时 30s+，
             超时直接返回空 DataFrame，不阻塞主流程。
    """
    import concurrent.futures
    try:
        def _fetch():
...
def kelly_position_advice(score: float, strategy: str = "", market_state: str = "",
    ...
    ★ v10.1（保本版）：
      - 新增 alert_layer 参数区分超早期/即将封板，应用对应仓位上限
      - 整合大盘涨停家数弱市折扣（WEAK_MARKET_ZT_THRESHOLD）
      - 整合炸板率高仓位折扣（BOMB_RATE_REDUCE_THRESHOLD）
      - 单笔硬上限：任何情况不超过 MAX_POSITION_PER_TRADE
    公式：Kelly = 胜率-(1-胜率)/盈亏比 × 评分系数 × 月份系数 × 市场折扣 × 赚钱效应系数
          → 再应用层级/炸板率/全局硬上限 clip
    """
    # 按策略取对应参数
    ...
    # ★ v10.1 保本：大盘涨停家数弱市折扣（涨停<30家时额外降仓）
    zt_note = ""
    if emotion:
        zt_count = emotion.get("zt_count", 99)
        if 0 < zt_count <= EXTREME_WEAK_ZT_THRESHOLD:
            pct_raw *= EXTREME_WEAK_POS_RATIO
            zt_note = f"涨停极少({zt_count}家)×{EXTREME_WEAK_POS_RATIO}"
        elif 0 < zt_count <= WEAK_MARKET_ZT_THRESHOLD:
            pct_raw *= WEAK_MARKET_POS_RATIO
            zt_note = f"涨停偏少({zt_count}家)×{WEAK_MARKET_POS_RATIO}"

    # ★ v10.1 保本：炸板率高时仓位折扣（分歧大=主力不护盘）
    bomb_note = ""
    if emotion:
        bomb_rate = emotion.get("bomb_open_rate", 0.0)
        if bomb_rate >= BOMB_RATE_STOP_THRESHOLD:
            pct_raw *= BOMB_RATE_STOP_RATIO
            bomb_note = f"炸板率{bomb_rate:.0%}×{BOMB_RATE_STOP_RATIO}"
        elif bomb_rate >= BOMB_RATE_REDUCE_THRESHOLD:
            pct_raw *= BOMB_RATE_REDUCE_RATIO
            bomb_note = f"炸板率{bomb_rate:.0%}×{BOMB_RATE_REDUCE_RATIO}"

    notes = " | ".join(filter(None, [month_note, market_note, heat_note, zt_note, bomb_note]))
    ...
    # ★ v10.1 保本：层级硬上限（超早期/即将封板专用上限）
    hard_cap = MAX_POSITION_PER_TRADE
    cap_note = ""
    if alert_layer == "super_early":
        hard_cap = MAX_POSITION_SUPER_EARLY
        cap_note = f"超早期上限{MAX_POSITION_SUPER_EARLY}%"
    elif alert_layer == "near_zt":
        hard_cap = MAX_POSITION_NEAR_ZT
        cap_note = f"追板上限{MAX_POSITION_NEAR_ZT}%"
    else:
        cap_note = f"单笔上限{MAX_POSITION_PER_TRADE}%"

    if pct > hard_cap:
        pct = hard_cap
        notes_str += f"，⚠️{cap_note}"
    else:
        notes_str += f"，{cap_note}"

    # 止损纪律提示
    sl_pct = INTRADAY_STOP_LOSS_NEAR_ZT if alert_layer == "near_zt" else INTRADAY_STOP_LOSS_PCT
    sl_note = f"当日浮亏{sl_pct:.0%}立刻出"

    return f"💰仓位建议：**{pct}%**（{strategy or '默认'}/{label}，Kelly公式{notes_str}）\n> ⚠️止损纪律：{sl_note}，不过夜"
...
def get_market_index() -> dict:
    ...
    # ── 北向资金（今日净流入）─────────────────────────────────
    # ★ v10.1：加8s超时保护，东方财富北向接口在境外极不稳定，失败时静默跳过不影响主流程
    import concurrent.futures as _cf
    try:
        def _fetch_north():
            return ak.stock_em_hsgt_north_net_flow_in(symbol="沪深港通")
        with _cf.ThreadPoolExecutor(max_workers=1) as _ex:
            _fut = _ex.submit(_fetch_north)
            try:
                north_df = _fut.result(timeout=8)
            except _cf.TimeoutError:
                log.debug("北向资金接口超时(8s)，跳过")
                north_df = None
...
def fetch_financial_news() -> list:
    """
    从东方财富获取最新财经快讯。
    返回 list of dict: {id, time, title, content, raw_code}

    ★ v10.1 简化：
      - 只保留 stock_news_em 主接口（快讯），超时 10s
      - 删除 stock_notice_report 备用接口：该接口在境外必失败且每次等15s，
        实测快讯失败时公告也同样失败（同一服务器），备用无意义
    """
    ...
    # 东方财富电报/快讯（10s超时，境外常超时，失败静默返回空列表）
    df = None
    try:
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
            fut = ex.submit(ak.stock_news_em, symbol="")
            try:
                df = fut.result(timeout=10)
            except concurrent.futures.TimeoutError:
                log.debug("快讯获取超时(10s)，跳过")
            except Exception as e:
                log.debug(f"快讯获取失败: {e}")
    except Exception as e:
        log.debug(f"快讯获取异常: {e}")
    ...
    try:
        ...
        for _, row in rt.iterrows():
            try:
                ...
                # ── 组装信号说明 ──────────────────────────────────────────
                reason_parts = [
                    f"{alert_level} {surge_tag}",
                    f"现价{price:.2f}涨{chg_pct:.1%} → 涨停{zt_price:.2f}（还差{gap_pct:.1f}%利润）",
                    f"量比{vol_ratio:.1f}x",
                    f"成交{amt_yi:.1f}亿",
                ]
                if open_chg >= 0.01:
...
def format_pre_zt_signal(sig: PreZtSignal, rank: int, emotion: dict = None,
    ...
    # 判断层级（用于仓位上限和止损）
    if sig.chg_pct < 4.0:
        alert_layer = "super_early"
        stop_pct = 0.03
    elif sig.chg_pct >= 7.0:
        alert_layer = "near_zt"
        stop_pct = 0.02
    else:
        alert_layer = "normal"
    ...
    # 利润空间与紧迫度
    if sig.gap_pct >= 7.0:
        urgency = f"💰利润空间大（还差**{sig.gap_pct:.1f}%**），可从容建仓"
    elif sig.gap_pct >= 4.0:
        urgency = f"⚡利润空间适中（还差**{sig.gap_pct:.1f}%**），快速决策"
    else:
        urgency = f"🚀即将封板（还差**{sig.gap_pct:.1f}%**），追板or放弃"
...
def push_pre_zt_signals(signals: list, market: dict, emotion: dict = None) -> None:
    """推送直线拉升预警信号（v10.1：保本版，三层分级+仓位上限+止损纪律）"""
    if not signals:
        return
    ...
    # ── 按预警层级分组 ──────────────────────────────────────────────
    super_early = [s for s in signals if s.chg_pct < 4.0]
    early_conf  = [s for s in signals if 4.0 <= s.chg_pct < 7.0]
    near_zt     = [s for s in signals if s.chg_pct >= 7.0]

    dist_parts = []
    if super_early: dist_parts.append(f"🔔超早期({len(super_early)}只·上限{MAX_POSITION_SUPER_EARLY}%仓)")
    if early_conf:  dist_parts.append(f"⚡强攻({len(early_conf)}只·上限{MAX_POSITION_PER_TRADE}%仓)")
    if near_zt:     dist_parts.append(f"🚀即将封板({len(near_zt)}只·上限{MAX_POSITION_NEAR_ZT}%仓)")
    dist_str = " / ".join(dist_parts) if dist_parts else f"共{len(signals)}只"

    # 大盘涨停家数警示
    zt_count   = _emotion.get("zt_count", 0)
    bomb_rate  = _emotion.get("bomb_open_rate", 0.0)
    risk_warns = []
    if 0 < zt_count <= EXTREME_WEAK_ZT_THRESHOLD:
        risk_warns.append(f"⚠️**今日涨停仅{zt_count}家（极弱市）**，仓位已自动降至Kelly×{EXTREME_WEAK_POS_RATIO}")
    elif 0 < zt_count <= WEAK_MARKET_ZT_THRESHOLD:
        risk_warns.append(f"⚠️今日涨停{zt_count}家（偏弱），仓位已自动降至Kelly×{WEAK_MARKET_POS_RATIO}")
    if bomb_rate >= BOMB_RATE_STOP_THRESHOLD:
        risk_warns.append(f"⚠️**炸板率{bomb_rate:.0%}（极高）**，分歧严重，仓位×{BOMB_RATE_STOP_RATIO}")
    elif bomb_rate >= BOMB_RATE_REDUCE_THRESHOLD:
        risk_warns.append(f"⚠️炸板率{bomb_rate:.0%}，主力不护盘，仓位×{BOMB_RATE_REDUCE_RATIO}")
    risk_str = "\n> ".join(risk_warns) + "\n\n" if risk_warns else ""

    top = signals[0]
    header = (
        f"# 🚀涨停预警（保本版·超早期+强攻+即将封板）\n\n"
        f"时间：{beijing_now().strftime('%Y-%m-%d %H:%M')} | "
        f"共 **{len(signals)}只** 冲板信号\n\n"
        f"{mkt_line}"
        f"> {dist_str}\n\n"
        f"> {risk_str}"
        f"> ⚡**超早期信号**（<4%）：量比≥5x，6~8%利润空间，**仓位上限{MAX_POSITION_SUPER_EARLY}%**\n\n"
    )
    body = ""
    for i, sig in enumerate(signals[:8], 1):
        body += format_pre_zt_signal(sig, i, emotion=_emotion, market_state=mkt_state) + "\n---\n"

    footer = (
        "\n> 📌**v10.1 保本操作原则**：\n"
        f"> 1. 🔔超早期（<4%）：单笔上限**{MAX_POSITION_SUPER_EARLY}%**，浮亏{INTRADAY_STOP_LOSS_PCT:.0%}立刻出\n"
        f"> 2. ⚡强攻（4%~7%）：单笔上限**{MAX_POSITION_PER_TRADE}%**，浮亏{INTRADAY_STOP_LOSS_PCT:.0%}立刻出\n"
        f"> 3. 🚀即将封板（>7%）：单笔上限**{MAX_POSITION_NEAR_ZT}%**，浮亏{INTRADAY_STOP_LOSS_NEAR_ZT:.0%}立刻出\n"
        "> 4. 量比由大变小 = 主力在出货，立刻观望；价格回落超高点2% = 信号失效\n"
        "> 5. 同日最多同时持有3只，总仓位不超过30%（分散风险）\n"
        "> 6. 止损后当日不再入场同只股票（情绪已破坏）\n"
        "> 7. 📌含近期涨停标记的股票有连板预期，但止损纪律不变"
    )

    title = (
        f"🚀涨停预警:{top.name}+{top.chg_pct:.1f}% 量{top.vol_ratio:.1f}x"
        f" 还差{top.gap_pct:.1f}%利润 [{now_str}]"
    )
    send_wx(title, header + body + footer)
...
def main():
    ...
    while True:
        try:
            ...
            # ★ 盘中：异动拉升扫描（量价齐升）
            # ────────────────────────────────────────────────────────────
            # ★ v10.1 保本版：盘中信号段落获取一次市场情绪供所有仓位建议使用
            _intraday_emotion: dict = {}
            if phase in ("morning", "afternoon"):
                try:
                    _intraday_emotion = get_market_emotion()
                except Exception:
                    _intraday_emotion = {}
            ...
            if phase in ("morning", "afternoon") and hm > 935:
                ...
                if new_pre_zt:
                    push_pre_zt_signals(new_pre_zt, market, emotion=_intraday_emotion)   # 传入emotion供保本仓位计算
                    for s in new_pre_zt:
